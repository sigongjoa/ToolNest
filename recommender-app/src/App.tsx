...
// Minimal Tool type definition for recommender-app
interface Tool {
  id: number;
  name: string;
  description: string;
...
const logger = {
  debug: (...args: any[]) => {
    console.debug('[RecommenderApp]', ...args);
  }
};
...
function App() {
  ...
  useEffect(() => {
    logger.debug('useEffect: 컴포넌트 마운트 시 WebLLM 엔진 초기화 시도');
      ...
      try {
        ...
        await initializeWebLLMEngine(
          (info: { progress: number; text: string }) => {
            ...
            logger.debug(`모델 로딩 진행: ${info.text} (${info.progress.toFixed(2)}%)`);
          }
        );
        setIsModelInitialized(true);
        logger.debug('WebLLM 엔진 초기화 및 모델 로딩 완료');
      } catch (err) {
        logger.debug('WebLLM 엔진 초기화 중 오류 발생:', err);
  ...
  const handleRecommend = useCallback(async () => {
    logger.debug('handleRecommend 함수 진입', { prefText });
    ...
    try {
      ...
      logger.debug('추천 성공:', { result });
    } catch (err) {
      logger.debug('추천 생성 중 오류 발생:', err);
      setError(`추천 생성 실패: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setIsRecommending(false);
      logger.debug('handleRecommend 함수 종료');
    }
  }, [prefText]);
        ...
        <section className="mb-8">
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              logger.debug('prefText 업데이트:', e.target.value);
            }}
          />
          <Button
