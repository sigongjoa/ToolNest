...
export default function AIToolRecommender() {
    ...
    const [webLLMLog, setWebLLMLog] = useState([]);
    ...
    useEffect(() => {
        ...
        if (!isWebLLMInitialized && !isWebLLMLoading) {
            ...
            initializeWebLLMEngine(initProgressCallback)
                ...
                setWebLLMLog((prev) => [...prev, `WebLLM 엔진 초기화 실패: ${error.message}`]);
                setIsWebLLMLoading(false);
            });
        }
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield loadAIData(); // Ensure data is loaded in recommender.ts
            console.debug('recommendAITools 호출 시작');
            const recommendedTools = yield recommendAITools(query);
            ...
            setWebLLMLog((prev) => [...prev, `추천 중 오류 발생: ${error.message}`]);
            setIsLoading(false);
        }
    });
        ...
        <div className="max-w-6xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-3">
                  ...
                  <Textarea id="query" value={query} onChange={(e) => {
            console.debug('쿼리 변경 감지', e.target.value);
            setQuery(e.target.value);
        }} placeholder="예: 문서 요약해 주는 AI 도구를 찾고 있습니다. 한글 지원이 되고, 무료로 사용할 수 있으면 좋겠어요. 또는, 이미지를 생성하고 편집하는 데 도움이 되는 AI 도구를 추천해 주세요." className="min-h-[120px] text-base resize-none border-slate-200 focus:border-blue-500 focus:ring-blue-500/20"/>
                </div>

                <Button type="submit" disabled={!query.trim() || isLoading || !isWebLLMInitialized} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 text-lg shadow-lg hover:shadow-xl transition-all duration-200">
                  {isLoading ? (<>
              ...
              {recommendations.length > 0 && (<div className="mt-8">
                  <h3 className="text-xl font-semibold text-slate-700 mb-4 flex items-center gap-2">
                    <Zap className="w-5 h-5 text-yellow-500"/>
                    추천 AI 도구
                  </h3>
                  <div className="grid grid-cols-1 gap-4">
                    {recommendations.map((tool, index) => (<Card key={index} className="border-l-4 border-blue-500 shadow-md hover:shadow-lg transition-all duration-200">
          ...
          <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
            <CardContent className="p-8">
              ...
              {allAITools.length === 0 && (<p className="text-slate-600">AI 도구를 불러오는 중입니다...</p>)}
              <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6">
                {allAITools.map((tool) => (<Card key={tool.id} className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-white/90 backdrop-blur-sm hover:scale-105">
        ...
        <div className="max-w-4xl mx-auto mt-8">
          ...
          <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
            <CardContent className="p-6 h-64 overflow-y-auto">
              ...
              {webLLMLog.map((log, index) => (<p key={index} className="font-mono text-sm text-slate-700">{log}</p>))}
            </CardContent>
          </Card>
        </div>
      </div>
